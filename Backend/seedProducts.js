...
// Each product gets 4 crop variants of the same photo for a realistic multi-angle feel
const u = (id) => [
    `https://images.unsplash.com/photo-${id}?w=500&h=700&fit=crop&crop=top&auto=format&q=80`,
    `https://images.unsplash.com/photo-${id}?w=500&h=700&fit=crop&crop=center&auto=format&q=80`,
    `https://images.unsplash.com/photo-${id}?w=500&h=700&fit=crop&crop=bottom&auto=format&q=80`,
    `https://images.unsplash.com/photo-${id}?w=600&h=800&fit=crop&crop=entropy&auto=format&q=80`,
];

// ─── Image pools per clothing type ────────────────────────────────────────────
const IMGS = {
    menTee:     ["1521572163474-6864f9cf17ab","1503341504253-dff4815485f1","1529374255404-311a2a4f1fd9","1596755094514-f87e34085b2c","1583743814966-8d4f4d0bd754","1562157873-818bc0726f68"],
    menShirt:   ["1584464491033-06628f3a6b7b","1600080972548-22b47b5d7a8d","1617196034183-421b4917c92d","1598033129183-c4f50c736f10"],
    menHoodie:  ["1556821840-3a63f15732ce","1515886657613-9f3515b0c78f","1578681994506-b8f463449011","1611312449408-fcf9c62f19ef"],
    menJacket:  ["1591047139829-d91aecb6caea","1548532928-b34e3be62fc6","1520975661132-815d7dd2b21a","1539533113208-f193a1f0cb5f"],
    menJeans:   ["1542219550-37153d387353","1560243563-062bfc001d68","1624378439432-ae1e0e2f20dc","1473966968600-fa4cebea5f78"],
    menShorts:  ["1617196034284-8b81f813c909","1506629082955-511b1aa562c8","1518199535-a54b6a2d9b4a","1503944583220-79d4dd7b3d10"],
    womenTop:   ["1496747986609-d963abfa5a9a","1572804013309-59a88b7e92f1","1539008835657-9e8e53f7c60e","1495385794356-15371f197762","1584361853901-dd1e9dd42a1e"],
    womenDress: ["1515372392135-5f240b866ef0","1585487000160-6ebcfceb0d03","1496360163847-0cf6bcffe7b1","1566174053879-31528523f8ae","1612336307429-8a06022a2596"],
    womenJeans: ["1541099649105-f69ad21f3246","1594938298603-c8148c4b4a3e","1619603364853-6b6bf56e9c2d","1604176424472-9d5e2e5adc8b"],
    womenSkirt: ["1617922001439-4a2e6562f328","1583744927625-a4d64f00cd04","1551854838-212c9b8d9d8f","1548545615-09af4b6c0d56"],
    kidsTop:    ["1519689680058-324335c77eba","1503944583220-79d4dd7b3d10","1518831959646-742c3a14ebf7","1519278409-1fd51b5a1a23"],
    kidsBottom: ["1503944583220-79d4dd7b3d10","1519689680058-324335c77eba","1591474200732-b3c6df8e72cb","1546961342-ea5f70fb9f2c"],
};

let imgCounts = {};
const pick = (pool) => {
    const key = pool.join(',').slice(0,20);
    imgCounts[key] = imgCounts[key] || 0;
    const id = pool[imgCounts[key] % pool.length];
    imgCounts[key]++;
    return id;
};
...
// ─────────────────────────────────────────────────────────────────────────────
// PRODUCTS — 108 items across 12 professional brands
// ─────────────────────────────────────────────────────────────────────────────
const products = [
    ...
    p("Adidas","Tiro 24 Track Jacket","Iconic Adidas Tiro track jacket with moisture-wicking AEROREADY technology. Zip pockets, tapered fit and bold 3-Stripes — perfect for training or streetwear.",2999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket,true),
    ...
    p("Adidas","Women's Dance Dress","Adidas flowing dance-inspired dress in moisture-absorbing fabric. Bold 3-Stripes detail and a sporty silhouette that transitions from studio to street.",2799,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Adidas","Classic Bomber Jacket","Premium Adidas Heritage Bomber in a smooth fabric with ribbed collar, cuffs and hem. Front zip closure and side zip pockets — a retro icon reinvented.",4499,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Nike","Club Fleece Pullover Hoodie","Made from soft midweight fleece for all-day comfort. Relaxed fit, kangaroo pocket and ribbed cuffs. The Nike Club Fleece Hoodie is a wardrobe cornerstone.",3499,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie,true),
    p("Nike","Tech Fleece Full-Zip Hoodie","Nike Tech Fleece delivers exceptional warmth with lightweight material. Engineered spacer fabric for targeted warmth with incredible softness.",5999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie),
    p("Nike","Windrunner Jacket","The iconic Nike Windrunner — lightweight ripstop fabric with a chevron design on the chest. Full-zip closure, hood and side pockets. A true Nike classic.",4999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Puma","Squad Men's Full-Zip Hoodie","Puma Squad hoodie in a warm cotton-blend fleece with full-zip closure. Side pockets and adjustable drawstring hood make it versatile for any occasion.",2799,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie),
    p("Puma","Teamliga Training Jacket","Lightweight Puma training jacket with dryCELL moisture management. Full-zip with stand collar and side pockets — game-ready from warm-up to final whistle.",3299,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Puma","Amplified Women's Hoodie","Oversized Puma Amplified hoodie with large graphic logo. Cotton-blend fleece with kangaroo pocket and adjustable hood — cosy streetwear done right.",2799,"Women","Topwear",SIZES_WOMEN_TOP,IMGS.womenTop,true),
    ...
    p("H&M","Oversized Printed Hoodie","H&M oversized hoodie with bold front graphic print. Warm cotton-blend fleece with spacious kangaroo pocket and a roomy hood for that street-ready look.",1999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie),
    p("H&M","Denim Jacket","H&M classic denim jacket in washed cotton denim. Button front closure with chest pockets and side pockets — a timeless wardrobe staple.",2799,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("H&M","Women's Floral Midi Dress","H&M floral-print midi dress in airy woven fabric. A-line silhouette with a V-neckline, puff sleeves and a tie waist — effortlessly feminine.",2499,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress,true),
    p("H&M","Women's Wrap Dress","H&M wrap dress with long sleeves and a flowy midi length. Adjustable self-tie waist and a flattering V-neckline — from brunch to evening.",2199,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    // ══════════════════════ ZARA ══════════════════════════
    p("Zara","Men's Structured Blazer","Zara structured blazer in premium fabric blend with notch lapels. Slim-cut with a single-button fastening and welt pockets — sharp, modern tailoring.",5999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket,true),
    ...
    p("Zara","Women's Printed Floral Midi Dress","Zara flowing midi dress with all-over floral print. V-neckline with ruffle trim, puff sleeves and a gathered skirt — romantic and feminine.",3499,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress,true),
    ...
    p("Zara","Women's Asymmetric Dress","Zara chic asymmetric hem dress in stretch crepe. Sleeveless with a side cutout detail — a minimalist statement piece for modern women.",3999,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Levi's","Trucker Jacket","The iconic Levi's Trucker Jacket in rigid denim with two chest flap pockets. A wardrobe staple since 1967 — pair it with everything.",4999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Uniqlo","3D Knit Crew-Neck Sweater","Uniqlo 3D Knit seamless construction for a premium, well-fitted look. Soft blended yarn with no side seams — a minimalist masterpiece.",3499,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie),
    ...
    p("Uniqlo","Women's Pile Fleece Sweatshirt","Uniqlo Pile Fleece pullover in ultra-soft, plush fabric. Crew neckline with a relaxed silhouette — the cosiest sweatshirt you'll ever own.",2799,"Women","Topwear",SIZES_WOMEN_TOP,IMGS.womenTop),
    p("Uniqlo","Kids' Soft Fleece Hoodie","Uniqlo Soft Fleece hoodie for kids in ultra-cosy plush fabric. Full-zip with adjustable hood and side pockets — warm and playful.",1299,"Kids","Topwear",SIZES_KIDS,IMGS.kidsTop),

    // ══════════════════════ MANGO ══════════════════════════
    p("Mango","Women's Satin Slip Dress","Mango elegant satin-finish slip dress with adjustable spaghetti straps. Midi length with a delicate lace trim — effortlessly luxurious.",3999,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress,true),
    p("Mango","Women's Floral Wrap Dress","Mango floral wrap dress in a soft crinkle fabric. V-neckline with long sleeves and a tie waist — a bohemian piece that transitions from day to night.",3299,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress,true),
    ...
    p("Mango","Women's Embroidered Midi Dress","Mango midi dress with delicate floral embroidery detail on the bodice. Lightweight woven fabric with puff sleeves — a romantic statement piece.",4499,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Jack & Jones","Premium Hoodie","Jack & Jones premium heavyweight hoodie in a brushed fleece. Full-zip or pullover with thick cotton construction for the colder months.",2799,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie,true),
    p("Jack & Jones","Denim Trucker Jacket","Jack & Jones washed denim trucker jacket with chest pockets and a button-through closure. A casual classic that layers over everything.",3499,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Jack & Jones","Men's Bomber Jacket","Jack & Jones bomber jacket in lightweight ripstop fabric. Ribbed collar, cuffs and hem with side zip pockets — classic cool.",4999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),

    // ══════════════════════ VERO MODA ══════════════════════════
    p("Vero Moda","Women's Smock Dress","Vero Moda feminine smock dress with elasticated chest panel and puff sleeves. Floral print on a soft woven fabric — charming and comfortable.",2799,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress,true),
    p("Vero Moda","Women's Wrap Midi Dress","Vero Moda wrap dress with a flattering v-neckline and self-tie waist. Midi length in a soft printed fabric — elegant yet relaxed.",3199,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Vero Moda","Women's Oversized Blazer","Vero Moda oversized blazer in a textured fabric blend. Notch lapels, two pockets and a relaxed silhouette — the season's most versatile piece.",4999,"Women","Topwear",SIZES_WOMEN_TOP,IMGS.womenTop),
    ...
    p("Vero Moda","Women's Floral Maxi Dress","Vero Moda maxi dress in a lightweight floral print chiffon. V-neckline with adjustable straps and a flowy A-line silhouette — bohemian perfection.",3799,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Campus Sutra","Men's Colourblock Hoodie","Campus Sutra colourblock hoodie in a warm cotton-blend fleece. Contrast panels, kangaroo pocket and a stylish oversized fit — fresh streetwear vibes.",1999,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie,true),
    p("Campus Sutra","Men's Printed Bomber","Campus Sutra printed bomber jacket with satin-feel exterior. Ribbed trims and bold all-over graphic print — a statement outerwear piece.",3299,"Men","Topwear",SIZES_MEN_TOP,IMGS.menJacket),
    ...
    p("Campus Sutra","Women's Crop Sweatshirt","Campus Sutra women's crop sweatshirt in cosy fleece fabric. Ribbed hem and cuffs with a crew neck — effortlessly cute for casual days.",1099,"Women","Topwear",SIZES_WOMEN_TOP,IMGS.womenTop),
    p("Campus Sutra","Women's Tie-Dye Midi Dress","Campus Sutra tie-dye midi dress in a vibrant, psychedelic palette. Soft jersey knit with a relaxed silhouette — festival-ready and free-spirited.",1799,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
    ...
    p("Roadster","Men's Sweatshirt","Roadster solid sweatshirt in warm fleece-back cotton. Ribbed cuffs and hem with a clean crew neckline — your most worn layer in cooler weather.",1499,"Men","Topwear",SIZES_MEN_TOP,IMGS.menHoodie),
    ...
    p("Roadster","Women's Printed Summer Dress","Roadster breezy summer dress with all-over print in lightweight cotton. Strappy neckline, tiered skirt — made for warm days and golden evenings.",1499,"Women","Dress",SIZES_WOMEN_TOP,IMGS.womenDress),
];
...
async function seed() {
    ...
    console.log(`\n🎉 Done! ${inserted} added, ${skipped} skipped. Total in DB: ${await Product.countDocuments()}`);
